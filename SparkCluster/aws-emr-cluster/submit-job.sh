...
aws emr add-steps --cluster-id j-F8H21XWXWAX5 --steps Type=Spark,Name="Spark application",ActionOnFailure=CONTINUE,\
...
aws emr add-steps --cluster-id j-F8H21XWXWAX5 --steps Type=Spark,Name="Spark application",ActionOnFailure=CONTINUE,\
Args="[--class,edu.spark.JavaSparkPi,--deploy-mode,cluster,--master,yarn,--conf,spark.yarn.submit.waitAppCompletion=false,s3://codelocation-emr-test/point.stream.classifier-1.0.jar]"
